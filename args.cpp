...
BOOL ParseCommandlineW(LPARGS args)
{
    ...
    //
    // point to first fileinfo and init. This will contain the entry for the output file that is the console, unless
    // redirected.
    //
    fi = &args->fi;
    fi->hFile = NULL;
    ...
    while (GetCommandLineTokenW(&lpToken))
    {
        ...
        else if (!lstrcmpiW(lpToken, L"/+")) // append
        {
            ...
            if (!CreateFileInfoStruct(&fi))
                ExitProcess(Perror((DWORD)NULL));
            fi->bAppend = TRUE;

            if (!CheckFileName(lpToken))
                ExitProcess(Perror(ERROR_INVALID_PARAMETER));
            if (!StringAllocW(&fi->lpFileName, lpToken))
                ExitProcess(Perror((DWORD)NULL));
        }
        else
        {
