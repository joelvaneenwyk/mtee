...
static BOOL GetSystemTimesAsUlargeInteger_(
    ULARGE_INTEGER *idleTime, ULARGE_INTEGER *kernelTime, ULARGE_INTEGER *userTime)
{
    FILETIME currentKernelTime;
    FILETIME currentIdleTime;
    FILETIME currentUserTime;

    BOOL rc = GetSystemTimes(&currentIdleTime, &currentKernelTime, &currentUserTime);
    if (rc != 0)
    {
...
BOOL cpuLoadGetCurrentCpuLoad(double *currentCpuLoad)
{
    ...
    BOOL rcOs = GetSystemTimesAsUlargeInteger_(&currentIdleTime, &currentKernelTime, &currentUserTime);
    if (0 != rcOs)
    {
