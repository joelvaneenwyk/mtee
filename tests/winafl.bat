...
:RunWinAfl
    ...
    set _dynamo=%_root%\tests\dynamorio\build\bin64
    ...
    if not exist "%_root%\tests\dynamorio\build" mkdir "%_root%\tests\dynamorio"

    cmake -S "%_root%\tests\dynamorio" -B "%_root%\tests\dynamorio\build" -G "Visual Studio 16 2019" -A x64 %_dynamorio_settings%
    if errorlevel 1 (
        echo Failed to generate `dynamorio` project files.
        exit /b 1
    )

    cmake --build "%_root%\tests\dynamorio\build" --config %_dynamorio_config%
    if errorlevel 1 (
    ...
    cmake -S "%_root%\tests\winafl" -B "%_root%\tests\winafl\build" -G "Visual Studio 16 2019" -A x64 %_winafl_settings% -DCMAKE_INSTALL_PREFIX="%_root%\tests\winafl\bin64"
    if errorlevel 1 (
        echo Failed to generate `winafl` project files.
        exit /b 1
    )

    cmake --build "%_root%\tests\winafl\build" --config %_winafl_config%
    if errorlevel 1 (
